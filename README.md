...
- **Rolling summary.** When you resume a session whose summary has gone stale, regenerates it during session start and folds it into the system prompt, so the model continues with prior context. Run `/summarize-session` to regenerate on demand.
...
|---|---|
| `/summarize-session` | Write a fresh summary of the current session now, ignoring the staleness window. |
...
| `PI_GENERATE_SUMMARY` | Generate the rolling summary |
| `PI_INJECT_SUMMARY` | Inject the summary into the system prompt |
| `PI_APPEND_MESSAGE` | The append-message commands |

Generate and inject are a pair: one writes `~/.pi/agent/summary.md`, the other reads it. Both must be on for the summary to reach the model.
...
| `PI_SUMMARY_STALENESS_DAYS` | `3` | How old a summary may get before it's regenerated on resume. |

### Summary prompt

Override the built-in prompt with a file at `prompts/session-summary.md`, read from the project `.pi/` first, then `~/.pi/`. It must contain the `{conversation_history}` placeholder.
